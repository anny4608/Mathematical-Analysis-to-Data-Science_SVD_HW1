...
# 使用彩色圖片
original_image = dog.astype(np.float64)
fileName = "dog_color"
original_image_path = f"./image/{fileName}.jpg"

# 儲存原始彩色圖片
Image.fromarray(dog).save(original_image_path)
...
for k in rangeArrK:
    # 對三個色彩通道分別進行 SVD
    R_compressed, sigma_R, norm_R, sigmak1_R, mse_R = svd_restore(original_image[:,:,0], k)
    G_compressed, sigma_G, norm_G, sigmak1_G, mse_G = svd_restore(original_image[:,:,1], k)
    B_compressed, sigma_B, norm_B, sigmak1_B, mse_B = svd_restore(original_image[:,:,2], k)
    
    # 合併三個通道成彩色圖片
    compressed_color = np.stack([R_compressed, G_compressed, B_compressed], axis=2)
    
    # 計算整體的指標（取三個通道的平均值）
    norm_val = (norm_R + norm_G + norm_B) / 3
    sigmak1 = (sigmak1_R + sigmak1_G + sigmak1_B) / 3
    mse_val = (mse_R + mse_G + mse_B) / 3
    
    # 儲存壓縮後的彩色圖片
    compressed_image_path = f"./image/{fileName}_{k}.jpg"
    Image.fromarray(compressed_color).save(compressed_image_path)
    compressedImageSize = round(os.stat(compressed_image_path).st_size / 1024, 2)
    
    # 計算 PSNR（對彩色圖片）
    psnr_val = psnr(original_image, compressed_color)
    ...
    title = f"k : {k}"
    plt.title(title)
    plt.imshow(compressed_color.astype(np.uint8))  # 彩色圖片不需要 cmap="gray"

plt.subplots_adjust(left=0.125, bottom=0.1, right=0.9, top=0.9, wspace=0.2, hspace=0.5)
plt.savefig('svd_hw1_compressed_images_color.png')
plt.show()

colorResult = np.array(results)

## Plotting Results ##
def showLinear(table, resize):
   fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(24,8),dpi=50,linewidth = 2)
   ...
   plt.tight_layout()
   plt.savefig('svd_hw1_linear_plots_color.png')
   plt.show()

showLinear(colorResult, 50)

## Display Results Table ##
df = pd.DataFrame(colorResult, columns=feature_names)
print(df)
dfi.export(df, 'svd_hw1_results_table_color.png')

## Plot PSNR and Compression Ratio vs. k ##
fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(12, 12))

# Subplot 1: PSNR vs. k
ax1.plot(colorResult[:, 0], colorResult[:, 8], 'o-')
ax1.set_title('PSNR vs. k (Color Image)')
ax1.set_xlabel('k value')
ax1.set_ylabel('PSNR (dB)')
ax1.grid(True)

# Subplot 2: Compression Ratio vs. k
ax2.plot(colorResult[:, 0], colorResult[:, 5], 'o-')
ax2.set_title('Compression Ratio vs. k (Color Image)')
ax2.set_xlabel('k value')
ax2.set_ylabel('Compression Ratio')
ax2.grid(True)

plt.tight_layout()
plt.savefig('k_vs_psnr_cr_color.png')
plt.show()